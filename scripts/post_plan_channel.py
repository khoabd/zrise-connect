#!/usr/bin/env python3
"""
post_plan_channel.py - Post AI execution plan via OpenClaw channel routing.
...
def format_plan_message(plan: dict, job_id: int, task_detail: dict = None) -> str:
    """
    Format execution plan as message.
...
def post_plan_to_channel(plan: dict, target: str = None, job_id: int = None, channel: str = None) -> dict:
    """
    Post execution plan via OpenClaw channel routing for human review.
    ...
    Example plan structure:
        {
            ...
            "steps": [
                {"description": "Step 1: Create auth module"},
                {"description": "Step 2: Add unit tests"},
            ],
            "priority": "high"
        }
    """
    try:
        ...
        # Format text (escape special chars, truncate)
        ...
        if result.get('success'):
            ...
            if task_id:
                add_task_log(
                    task_id, 'plan_posted_channel', 'coder',
                    {
                        'job_id': job_id,
                        'target': target,
                        'channel': channel,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Post plan via OpenClaw channel')
    ...
    result = post_plan_to_channel(test_plan, target=args.target, job_id=args.job_id, channel=args.channel)
    
    if result.get('success'):
        print(f"✅ Posted plan via OpenClaw channel")
